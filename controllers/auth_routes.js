...
auth_router.post('/register', isLoggedIn, (req, res) => {
    ...
    User.findOne({
        ...
        if (user) {
            req.sessions.errors = ['A user already exists with that email address.'];
            return res.redirect('/register');
        }
        ...
        User.create(req.body)
            .then(new_user => {
                ...
                console.log(err);
                req.sessions.errors = err.errors.map(e => e.message);
...
auth_router.post('/login', isLoggedIn, (req, res) => {
    ...
    if (!email || !password) {
        req.session.error = ['Please check your credentials and try again'];
    ...
    User.findOne({
        ...
        if (!user) {
            req.sessions.errors = ['NO user account found matching that email address.'];
            return res.redirect('/login');
        }
